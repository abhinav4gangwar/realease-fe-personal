...
export const useUploadHandler = ({ onSuccess, onClose }: UseUploadHandlerProps) => {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[], fileRejections: any, event: any) => {
    ...
    if (event.dataTransfer?.items) {
      ...
      items = processedItems.filter((item): item is FileItem => item !== null)
    } else {
      items = buildFileTreeFromInput(acceptedFiles)
    }
    items.sort((a, b) => {
  ...
  const handleFilesFromInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) setUploadedFiles(prev => [...prev, ...buildFileTreeFromInput(files)])
    event.target.value = ""
  }
  ...
  const updateFileMetadata = (path: string, field: "propertyId" | "tags", value: string) => {
    const updateInTree = (items: FileItem[]): FileItem[] => {
      return items.map((item) => {
        if (item.path === path) return { ...item, [field]: value }
        if (item.children) return { ...item, children: updateInTree(item.children) }
        return item
      })
    }
    setUploadedFiles(updateInTree)
  }
  ...
  return {
    ...
    getRootProps, getInputProps, isDragActive, openFileDialog, openFolderDialog, handleFilesFromInput,
